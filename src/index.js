...
async function validate_parameters() {
	...
	if (args.length < 3)
		ErrorOut(
			"Invalid arguments use the -help flag to see all possible options."
		);
}
...
async function validateParam() {
	...
	if (args.length != 4)
		ErrorOut(
			"Invalid project type was passed in. Expected either -js or -ts as a cli arg. Followed by a project name. ex:: npx electron-svelte-app -ts my-app"
		);
	...
	if (existsSync(folderLoc) && folderLoc != cwd()) {
		...
		if (files.length > 0) {
			ErrorOut(
				"Cannot create template inside non-empty folder: \n - " + folderLoc
			);
		}
	}

	if (
		args[2].toLowerCase().includes("ts") ||
		args[2].toLowerCase().includes("type")
	) {
		console.log("Creating a typescript-electron-svelte project.");
		out = { out: folderLoc, src: Locations.typescript };
	} else if (
		args[2].toLowerCase().includes("js") ||
		args[2].toLowerCase().includes("java")
	) {
		console.log("Creating a javascript-electron-svelte project.");
		out = { out: folderLoc, src: Locations.javascript };
	} else {
		ErrorOut(
			"Invalid project type was passed in. Expected either -js or -ts as a cli arg."
		);
	}
...
async function createTemplate(
	location_src = Locations.typescript,
	location_dest = ""
) {
	try {
